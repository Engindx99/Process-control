...
def fast_internal_predict(temp, o2, fuel, fan):
    ...
    if heat_gain > 2500: heat_gain = 2500
...
@njit
def fast_cost(u, horizon, temp0, o20, temp_target, last_fuel, last_fan, w_mse, w_o2, w_energy, w_smooth):
    fuel_seq = u[:horizon]
    fan_seq = u[horizon:]
    ...
    for k in range(horizon):
        ...
        if k == 0:
            cost += w_smooth * ((fuel_seq[k] - last_fuel) ** 2 + 0.1 * (fan_seq[k] - last_fan) ** 2)
        else:
            cost += w_smooth * ((fuel_seq[k] - fuel_seq[k-1]) ** 2 + 0.05 * (fan_seq[k] - fan_seq[k-1]) ** 2)
    return cost
...
class MPC:
    def __init__(self, config):
        ...
        self.temp_target = float(self.config["setpoint"])
    ...
    def optimize(self, plant):
        ...
        # İlk tahmin dizisi (Sıcak başlangıç için mevcut değerler kullanılır)
        x0 = np.concatenate([
        ...
        # Optimizasyonu Numba destekli fonksiyona yönlendiriyoruz
        res = minimize(
            fast_cost,
            x0,
            args=(self.horizon, temp0, o20, self.temp_target, self.last_fuel, self.last_fan, 
        ...
        if self.step_count % 100 == 0:
            ...
            print(f"🚀 MPC {self.step_count} | T: {temp0:.2f} | FUEL: {fuel_cmd:.2f} | ERROR: {error:.2f}")
