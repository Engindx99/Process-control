...
class MPC:
    def __init__(self, config_input: Union[str, Dict[str, Any]] = "config.yaml"):
        ...
        # 2. Ufuk ve Hedef Ayarları
        self.Np = int(mpc_cfg.get("prediction_horizon", 130)) 
        ...
        self.k_heat_plant = 0.351  
        self.C_th_plant = 740.0  
        
        # 3. Limitler (Config'den çekiliyor)
        self.fuel_min = float(mpc_cfg.get("fuel_min", 10.0))
        self.fuel_max = float(mpc_cfg.get("fuel_max", 25.0))
        self.fan_min = float(sys_cfg.get("fan_min", 50.0))
        self.fan_max = float(sys_cfg.get("fan_max", 1200.0)) 
        
        # Değişim hızı kısıtları (Slew rate limit)
        self.max_df = float(mpc_cfg.get("max_delta_fuel", 0.45))
        self.max_dfan = float(mpc_cfg.get("max_delta_fan", 30.0))

        # 4. Ağırlıklar (Cost Function Weights - Config'den çekiliyor)
        self.W_temp = float(mpc_cfg.get("weight_temp", 90000.0))
        self.W_o2 = float(mpc_cfg.get("weight_o2", 4000.0))
        self.W_fuel_chg = float(mpc_cfg.get("weight_fuel_chg", 350000.0))
        self.W_fan_chg = float(mpc_cfg.get("weight_fan_chg", 15000.0))

        # Başlangıç Değerleri
        self.last_fuel = float(mpc_cfg.get("initial_fuel", 18.5))
    ...
    def _setup_solver(self):
        ...
        self.P_fuel_hist = self.opti.parameter(10)     
        ...
        for k in range(self.Np):
            ...
            # --- 1. Hava Akışı ve Basınç Dinamiği (Plant Uyumlu) ---
            ...
            # --- 2. Yanma ve Isı Dengesi (Plant ile Eşitlendi) ---
            ...
            conv_loss = 0.0038 * (1.0 + 0.6 * ca.tanh((u_k[1] - 900.0) / 2070.0)) * (temp_k - 25.0)
            
            dT = (heat_gen - (rad_loss + conv_loss)) / self.C_th_plant
            next_temp = temp_k + dT
            
            # --- 3. O2 Dinamiği (Plant ile Eşitlendi) ---
            next_o2 = o2_k + 0.1 * (0.21 * air_flow - 0.031 * eff_fuel * eff + (2.2 - o2_k)/6.0)
        ...
        # Delta kısıtları (Dinamik config değerleri ile)
